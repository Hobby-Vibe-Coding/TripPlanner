...
const ALLOWED_MIME = new Set(["image/jpeg", "image/png", "image/gif", "image/webp", "image/heic", "image/heif"]);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("[drive/upload] uncaught:", err);
...
async function _handler(req, res) {
  ...
  if (!ALLOWED_MIME.has(mimeType)) return res.status(400).json({ error: "Only images can be uploaded here." });
  if (fileSize > MAX_BYTES) return res.status(413).json({ error: "File too large (max 20 MB)" });
  ...
  if (!driveRes.ok) {
    ...
    const msg = driveData?.error?.message || "Drive upload failed";
